...
func main(){
	if len(os.Args)<2 || len(os.Args)>4{
		...
		fmt.Println(os.Args[0]+" [dllName] [Args(not necessary)] [entryPoint(not necessary)]")
		os.Exit(1)
	}
	filename :=os.Args[1]
	execArgs := ""
	...
	sc, err:= ShellcodeRDIFromBytes(plaintext,funcName,execArgs)
	if err != nil {
		log.Panic(err)
	}

	dir := path.Dir(filename)
	outName := strings.Replace(path.Base(filename), ".dll", ".bin", 1)
	filePath := filepath.Join(dir, outName)
	err = ioutil.WriteFile(filePath, sc, 0700)
	if err != nil {
		log.Panic(err)
	}
